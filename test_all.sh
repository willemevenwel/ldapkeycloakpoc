...
keycloak_health=$(curl -s -o /dev/null -w "%{http_code}" ${KEYCLOAK_URL}/health/ready 2>/dev/null)
if [ "$keycloak_health" = "200" ]; then
    check_result "pass" "${MAGENTA}Keycloak${NC} service health" "ready and accessible"
else
    # Try alternative health check
    keycloak_alt=$(curl -s -o /dev/null -w "%{http_code}" ${KEYCLOAK_URL}/realms/master 2>/dev/null)
...
if [ -n "$REALM_NAME" ]; then
    ...
    realm_check=$(curl -s "${KEYCLOAK_URL}/realms/${REALM_NAME}" 2>/dev/null)
...
if [ -n "$REALM_NAME" ]; then
    ...
    if [ -n "$admin_token" ]; then
        ...
        ldap_providers=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/components?type=org.keycloak.storage.UserStorageProvider" \
        ...
        users_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/users?max=100" \
            -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
...
if [ -n "$REALM_NAME" ] && [ -n "$admin_token" ]; then
    ...
    realm_config_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}" \
        -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
    
    if [ -n "$realm_config_response" ]; then
        # Test 1: Check server-level Organizations feature
        server_features_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/serverinfo" \
            -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
        ...
        org_api_response=$(curl -s -w "%{http_code}" -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/organizations" \
        ...
        if [ "$org_api_response" = "200" ] && [ -f /tmp/org_api_check.json ]; then
            ...
            if [ "$org_count" -gt 0 ]; then
                ...
                all_idps_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/identity-provider/instances" \
                    -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
                ...
                if [ "$mock_idp_count" -gt 0 ]; then
                    ...
                    if command -v jq >/dev/null 2>&1; then
                        ...
                        for org_id in $org_ids; do
                            ...
                            org_idp_response=$(curl -s -w "%{http_code}" -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/organizations/${org_id}/identity-providers" \
                                -H "Authorization: Bearer ${admin_token}" \
...
if [ -n "$REALM_NAME" ] && [ -n "$admin_token" ]; then
    ...
    web_client_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/clients?clientId=shared-web-client" \
        -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
    ...
    if echo "$web_client_response" | grep -q "shared-web-client"; then
        ...
        if [ -n "$WEB_CLIENT_UUID" ]; then
            # Get client secret
            web_client_secret_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/clients/${WEB_CLIENT_UUID}/client-secret" \
                -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
    ...
    api_client_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/clients?clientId=shared-api-client" \
        -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
...
echo ""
    ...
    if [ -n "$WEB_CLIENT_UUID" ]; then
        ...
        mappers_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/clients/${WEB_CLIENT_UUID}/protocol-mappers/models" \
            -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
...
if [ -n "$REALM_NAME" ]; then
    ...
    if [ -n "$admin_token" ]; then
        roles_response=$(curl -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/roles" \
            -H "Authorization: Bearer ${admin_token}" 2>/dev/null)
